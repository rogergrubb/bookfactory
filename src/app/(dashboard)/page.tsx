...
import {
  BookOpen, PenTool, BarChart3, TrendingUp, Clock, Target,
  Sparkles, Plus, ArrowRight, Calendar, FileText, Users,
  Flame, Award, DollarSign, Eye, ChevronRight
} from 'lucide-react';
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="space-y-8 lg:col-span-2">
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                ...
                <span className={cn(
                  ...
                  goalMet ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                )}>
              ...
              <div className="h-4 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                <motion.div
                  ...
                  className={cn(
                    'h-full rounded-full',
                    goalMet ? 'bg-gradient-to-r from-emerald-500 to-teal-500' : 'bg-gradient-to-r from-violet-500 to-indigo-600'
                  )}
                />
              </div>
              <div className="mt-4 flex items-center justify-between">
                <Link
                  href="/write/1"
                  className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-lg hover:shadow-xl"
                >
                ...
                <button className="flex items-center gap-1 text-sm text-slate-500 hover:text-violet-600">
                  <Target className="h-4 w-4" /> Adjust goal
                </button>
              </div>
            </motion.div>
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                ...
                <Link href="/books" className="flex items-center gap-1 text-sm text-violet-600 hover:text-violet-700">
              ...
              <div className="space-y-4">
                {mockRecentBooks.map((book) => (
                  <Link
                    key={book.id}
                    href={`/write/${book.id}`}
                    className="flex items-center gap-4 rounded-xl border border-slate-100 p-4 transition-all hover:border-violet-200 hover:bg-violet-50/50 dark:border-slate-800 dark:hover:border-violet-800 dark:hover:bg-violet-950/20"
                  >
                    <div className="flex h-14 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500 to-indigo-600 text-sm font-bold text-white">
                      {book.title.charAt(0)}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={cn(
                          ...
                          book.status === 'published' ? 'bg-emerald-100 text-emerald-700' : 'bg-violet-100 text-violet-700'
                        )}>
                          {book.status}
                        </span>
                      </div>
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {quickActions.map((action) => (
                  <Link
                    ...
                    href={action.href}
                    className={cn(
                      'flex flex-col items-center gap-2 rounded-xl bg-gradient-to-br p-4 text-white transition-transform hover:scale-105',
                      action.color
                    )}
                  >
                ...
                {mockActivity.map((activity, index) => (
                  <div key={index} className="flex gap-3">
                    <div className={cn(
                      ...
                      activity.type === 'writing' && 'bg-violet-500',
            ...
            <motion.div
              ...
              className="rounded-2xl bg-gradient-to-br from-violet-500 to-indigo-600 p-6 text-white"
            >
              <div className="mb-3 flex items-center gap-2">
                <Sparkles className="h-5 w-5" />
                <span className="font-medium">Daily Writing Tip</span>
              </div>
              <p className="text-sm text-white/90">
                "The first draft is just you telling yourself the story." — Terry Pratchett
              </p>
              <button className="mt-4 flex items-center gap-1 text-sm text-white/80 hover:text-white">
                Get more tips <ArrowRight className="h-4 w-4" />
              </button>
            </motion.div>
          </div>
        </div>
